...
export const detectUserCurrency = async (): Promise<Currency> => {
  // Try multiple geo-detection APIs with fallbacks
  const apis = [
    {
      url: "https://ipwho.is/",
  ...
  for (const api of apis) {
    try {
      ...
      if (country) {
        ...
        // Other Arab countries default to SAR
        const sarCountries = ["SA", "AE", "KW", "BH", "QA", "OM"];
        if (sarCountries.includes(country)) return "SAR";
        // Egypt
        if (country === "EG") return "EGP";
        // Everyone else gets SAR as default
        return "SAR";
      }
    } catch {
      // Try next API
  ...
  // If all APIs fail, default to SAR
  return "SAR";
};
